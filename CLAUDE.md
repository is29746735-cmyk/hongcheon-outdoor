...
- 반경: 입력 8–14px · 카드 12px · 시트 16px · 토글 pill · 그림자는 단일 레이어 순흑(컬러 틴트 금지)
- 핵심 수치(거리·평점·요금)는 700 + 고정폭(tabular) numerals
- 보이스: 한국어 1차, 차분·단정, **검증된 사실만**(과장·근사 금지)

전체 토큰·컴포넌트·상태·모션 규격은 `DESIGN.md` 참조.
(이 블록은 oh-my-design이 관리합니다. 새 레퍼런스로 다시 잡으려면 omd:init, 토큰 동기화는 omd:sync.)
<!-- omd:managed:end -->
